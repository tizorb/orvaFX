...
// Maneja el efecto de inicialización y escucha de sesión
  ...
  useEffect(() => {
    ...
    const performLoadInitialSession = async () => {
      try {
        ...
        console.error("Error in performLoadInitialSession:", err);
      } finally {
...
// Acciones de usuario (login, register)
const useAuthUserActionsInternal = (setLoading, setUser, setMfaChallenge, t) => {
  const loginUser = useCallback(async (identifier, password) => {
    ...
    let result;
    try {
      result = await signInWithIdentifierPassword(identifier, password, t);
  ...
  const registerUser = useCallback(async (username, email, password, referralCodeInput = null, role = 'user') => {
    ...
    let result;
    try {
      result = await signUpWithEmailPassword(username, email, password, referralCodeInput, role, t);
      if (result.success && result.profile) {
        setUser(result.profile);
        saveToLocalStorage(STORAGE_KEYS.CURRENT_USER, result.profile);
      } else {
...
const useAuthProfileActionsInternal = (setLoading, setUser, t) => {
  const updateUserSubscriptionCtx = useCallback(async (userId, subscriptionTier, isSubscribed, subscriptionEndDate) => {
    ...
    let result;
    try {
      result = await updateUserSubscriptionInSupabase(userId, subscriptionTier, isSubscribed, subscriptionEndDate);
      ...
      console.error("Error during updateUserSubscriptionCtx:", err);
    } finally {
      setLoading(false);
    }
    return result;
  ...
  const requestWithdrawalCtx = useCallback(async (userId, amount, walletAddress) => {
    ...
    let result;
    try {
      result = await requestWithdrawalInSupabase(userId, amount, walletAddress, t);
      if (result.success && result.updatedProfile) {
        setUser(prevUser => {
          if (prevUser?.id === userId || prevUser?.auth_user_id === userId) {
            const updatedUser = { ...prevUser, ...result.updatedProfile };
      ...
      console.error("Error during requestWithdrawalCtx:", err);
    } finally {
      setLoading(false);
    }
    return result;
  ...
  const addReferralCodeToExistingUserCtx = useCallback(async (userId, referralCodeInput) => {
    ...
    let result;
    try {
      result = await applyReferralCodeInSupabase(userId, referralCodeInput, t);
      if (result.success && result.updatedProfile) {
        setUser(prevUser => {
          if (prevUser?.id === userId || prevUser?.auth_user_id === userId) {
            const updatedUser = { ...prevUser, ...result.updatedProfile };
      ...
      console.error("Error during addReferralCodeToExistingUserCtx:", err);
    } finally {
      setLoading(false);
    }
    return result;
  ...
  const purchasePlanCtx = useCallback(async (userId, username, email, subscriptionTier, amount, transactionHash, paymentProofUrl) => {
    ...
    let result;
    try {
      result = await purchasePlanRequest(userId, username, email, subscriptionTier, amount, transactionHash, paymentProofUrl, t);
    } catch (err) {
      console.error("Error during purchasePlanCtx:", err);
    } finally {
      setLoading(false);
    }
    return result;
...
const useAuthMfaActionsInternal = (setLoading, setMfaChallenge, t, refreshUserProfileInternal) => {
  const enrollMfa = useCallback(async () => {
    ...
    let result;
    try {
      result = await enrollNewMfaFactor(t);
    } catch (err) {
      console.error("Error during enrollMfa:", err);
    } finally {
      setLoading(false);
    }
    return result;
  ...
  const challengeAndVerifyMfa = useCallback(async (factorId, code) => {
    setLoading(true);
    let challengeResult, verifyResult;
    try {
      challengeResult = await createMfaChallenge(factorId, t);
      if (!challengeResult.success) {
        return challengeResult;
      }
      verifyResult = await verifyMfaChallenge(factorId, challengeResult.challengeId, code, t);
      ...
      console.error("Error during challengeAndVerifyMfa:", err);
    } finally {
      setLoading(false);
    }
  ...
  const unenrollMfa = useCallback(async (factorId) => {
    ...
    let result;
    try {
      result = await unenrollMfaFactor(factorId, t);
      if (result.success) {
        await refreshUserProfileInternal();
      }
    } catch (err) {
      console.error("Error during unenrollMfa:", err);
    } finally {
      setLoading(false);
    }
    return result;
  ...
  const listMfaFactors = useCallback(async () => {
    ...
    let result;
    try {
      result = await listUserMfaFactors(t);
    } catch (err) {
      console.error("Error during listMfaFactors:", err);
    } finally {
      setLoading(false);
    }
    return result;
...
export const AuthProvider = ({ children }) => {
  ...
  const performLogoutInternal = useCallback(async (doNotSetLoading = false) => {
    try {
      ...
      console.error("Error during performLogoutInternal:", err);
    } finally {
  ...
  const refreshUserProfileInternal = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error during refreshUserProfileInternal:", err);
      await performLogoutInternal(true);
    } finally {
      setLoading(false);
    }
